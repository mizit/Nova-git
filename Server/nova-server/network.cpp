...
void net_send_engine(MySocket* socket, CShip* ship)
{
    ...
    int runner = 3;
    runner = SetToRawData(data, runner, ship->main_drive.Calculation());
    runner = SetToRawData(data, runner, ship->man_drive.Calculation());
    runner = SetToRawData(data, runner, ship->back_drive.Calculation());
    runner = SetToRawData(data, runner, ship->mass.Calculation());
    data[0] = (unsigned char)(runner & 0xFF);
    data[1] = (unsigned char)((runner & 0xFF00) << 8);
    socket->MyWrite((char*)data, runner - 2);
}
...
void net_send_navigation(MySocket* socket, CShip* ship)
{
    ...
    data[2] = NET_SHIP_DATA;
    int runner = 3;
    runner = SetToRawData(data, runner, ship->radar_range.Calculation());
    runner = SetToRawData(data, runner, ship->radio_range.Calculation());
    runner = SetToRawData(data, runner, ship->system_level.Calculation());
    runner = SetToRawData(data, runner, ship->main_drive.Calculation());
    runner = SetToRawData(data, runner, ship->man_drive.Calculation());
    runner = SetToRawData(data, runner, ship->back_drive.Calculation());
    runner = SetToRawData(data, runner, ship->mass.Calculation());
